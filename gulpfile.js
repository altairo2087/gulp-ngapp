...
(function() {
  'use strict';
  var DIST_PATH, ENV, ENV_CURRENT, IMAGES, OPEN_BROWSER, ORDER_VENDOR_CSS, ORDER_VENDOR_JS, PORT, PUBLIC_PATH, Q, SERVER_WATCH, bower, browserSync, build, clean, coffee, css, filter, gulp, html, images, inject, jade, jadeWatch, js, opts, orderedCustomCss, orderedCustomJs, orderedVendorCss, orderedVendorJs, plugins, ref, sass, server, task, tasks, watch,
  ...
  inject = function(src) {
    ...
    return src.pipe(filterInject).pipe(plugins.inject(orderedVendorCss(), {
      name: 'bower',
      relative: true
  ...
  jade = function() {
    var src;
    src = gulp.src(DIST_PATH + "/**/*.jade").pipe(plugins.jade()).pipe(plugins.prettify({
      indent_size: 2
